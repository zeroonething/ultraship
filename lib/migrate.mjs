...
import { FRAMEWORK_VERSION, SCHEMA_VERSION } from './init.mjs';
...
// A pre-versioned or hand-made workspace may lack schema_version. Default it to
// the baseline so the workspace declares which contract it is on. Present values
// are left as-is; a shape change would bump SCHEMA_VERSION and add a step here.
function syncSchemaVersion(p) {
  const config = readYaml(p.config);
  if (config.schema_version === SCHEMA_VERSION) return { from: config.schema_version, to: config.schema_version, changed: false };
  const from = config.schema_version ?? null;
  config.schema_version = SCHEMA_VERSION;
  writeYaml(p.config, config);
  return { from, to: SCHEMA_VERSION, changed: true };
}

// The 2.0 upgrade step. Committing at checkpoints is the new default, but a
// workspace that predates 2.0 must not acquire it silently — only init writes the
// field, so an absent one means the workspace was created before this release.
// Pinning it to 'off' is what lets the default be on without surprising anyone.
function syncCommitPolicy(p) {
  const config = readYaml(p.config);
  if (config.commit_policy !== undefined) {
    return { from: config.commit_policy, to: config.commit_policy, changed: false };
  }
  config.commit_policy = 'off';
  writeYaml(p.config, config);
  return { from: null, to: 'off', changed: true };
}

export function migrate(root) {
  const p = paths(root);
  const schema_version = syncSchemaVersion(p);
  const commit_policy = syncCommitPolicy(p); // before the version sync, which erases the evidence
  ...
  if (!hadLegacy) return { migrated: false, reason: 'already per-product', product: null, lifecycle: null, framework_version, schema_version, commit_policy };
  ...
  return { migrated: true, product, lifecycle, framework_version, schema_version, commit_policy };
}
