...
import { snapshot, productState } from './state.mjs';
...
// Canonical YAML uses folded scalars for prose, which round-trip with newlines
// and would break a Markdown row across lines. Pipes would end the cell early.
// Backslashes escape in one pass with the pipes: escaping pipes alone turns a
// prose `\|` into `\\|`, a literal backslash beside a live cell-ending pipe.
function cell(value) {
...
function workspaceBrief(root, p) {
  ...
  if (!products.length) {
    ...
    for (const id of products) {
      ...
      const releases = releasesOf(p, id);
      const latest = releases.length ? releases[releases.length - 1].version : null;
      lines.push(
        `| ${cell(id)} | ${cell(productState(p, id))} | ${cell(latest)} | ${cell(doc?.classification)} | ${cell(doc?.mvp_boundary)} |`,
      );
    }
  }
  ...
  lines.push(sources('workspace.yaml', 'products/<id>/product.yaml', 'products/<id>/lifecycle.yaml'));
  return lines.join('\n');
}
  ...
  if (!products.length) {
    ...
    for (const id of products) {
      ...
      for (const entry of roadmap.versions) {
        lines.push(
          `| ${cell(entry.version)} | ${cell(entry.outcome)} | ${cell(entry.detail)} | ${cell(entry.status)} |`,
        );
      }
      lines.push('');
    }
...
function activeReleases(root, p) {
  ...
  for (const id of listProducts(p)) {
    ...
    const c = active.constraints;
    if (c && (c.time || c.budget || c.capacity)) {
      lines.push(
        `**Constraints** (${c.source}): time ${cell(c.time)}, budget ${cell(c.budget)}, capacity ${cell(c.capacity)}`,
        '',
      );
    }
...
function releaseHistory(root, p) {
  ...
  for (const id of listProducts(p)) {
    ...
    for (const { version, doc } of released) {
      lines.push(
        `| ${cell(version)} | ${cell(doc.released_at)} | ${cell(doc.mode)} | ${cell(doc.delivered)} |`,
      );
    }
    lines.push('');
