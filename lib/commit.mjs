...
export const ALLOWED_GIT = Object.freeze(['rev-parse', 'status', 'add', 'commit']);
...
export function commitCheckpoint(root, { checkpoint, product, version, task } = {}) {
  ...
  const status = runGit(['status', '--porcelain', '--', ...staged], projectRoot);
  if (status.exit !== 0) return { ...base, staged, reason: `git status failed — ${status.stderr}` };
  if (!status.stdout) return { ...base, staged, reason: 'No change under this checkpoint\'s paths.' };

  const add = runGit(['add', '--', ...staged], projectRoot);
  if (add.exit !== 0) {
    return { ...base, staged, ok: false, reason: `git add failed — ${add.stderr}` };
  }
  ...
  const commit = runGit(['commit', '--only', ...message, '--', ...staged], projectRoot);
  if (commit.exit !== 0) {
    return { ...base, staged, ok: false, subject, reason: `git commit failed — ${commit.stderr || commit.stdout}` };
  }

  const sha = runGit(['rev-parse', 'HEAD'], projectRoot);
  return { ...base, committed: true, ok: true, subject, sha: sha.stdout || null, staged };
}
