...
- CLI exit codes: `0` success, `1` validation or usage failure, `2` internal error.
...
Note: `minLength` is not in the interpreter's supported keyword set, so it is documentation only on `outcome`. The `required` array is what actually enforces presence, which is what the test asserts.
